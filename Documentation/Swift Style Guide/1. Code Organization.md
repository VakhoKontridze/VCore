...
Members declared within a non-`final` `class`es execute using a dynamic dispatch. And, members declared in an extension execute using a static dispatch. If extra performance is required, this may seem as a viable option.
...
If, however, you do not intend for the extension methods to be overridden, mark them as `final`. But, marking them as `final` eliminates the need for an extension.
