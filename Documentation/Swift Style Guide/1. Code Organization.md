...
However, there are pitfalls to this approach. Subclassing, and subsequently overriding a member declared within the extension requires an `@objc` annotation. These methods execute via dynamic dispatch, and actually worsen performance, since message-based dynamic dispatch is slower than table-based one.
