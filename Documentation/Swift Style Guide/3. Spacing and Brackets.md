...
- [Multi-Line Expressions in Assignments](#multi-line-expressions-in-assignments)
...
Generic types should be spaced out with a single space.
...
Conditional compilation flags should be indented at top-level, regardless of current indentation:
...
Long arithmetic expressions should be split to several lines to make them easier to read.
...
Long ternary expressions should be split to several lines to make them easier to read.

For additional info, refer to "Ternary Operators" section in "Context and Clarity" chapter.
