...
func getObject() -> SomeObject {
    let sourceA: [String: Int] = { ... }()
    
    let sourceB: [String: Int] = { ... }()

#if os(macOS)
    let sourceC: [String: Int] = { ... }() // This variable can only be used on `macOS`
#endif

    return SomeObject(
        a: {
            if let value = sourceA["a"] { return value }
            if let value = sourceB["a"] { return value }
#if os(macOS)
            if let value = sourceC["a"] { return value }
#endif
            return nil
        }(),
        b: { // `"b"` isn't expected in `sourceC`
            if let value = sourceA["b"] { return value }
            if let value = sourceB["b"] { return value }
            return nil
        }()
    )
}
