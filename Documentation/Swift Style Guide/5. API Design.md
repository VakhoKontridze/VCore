...
In some cases API can be significantly improved, or made more flexible, by choosing to use `struct`s instead of `enum`s. There are several reasons for this.
...
#### To Introduce Hidden Storage

You can introduce hidden storage to `enum`s:

```swift
let someConfiguration: Configuration = .first

public struct Configuration {
    // Hidden `enum` for switch-casing.
    // If unnecessary, simple identifying property can be used to tell instances apart.
    var _configuration: _Configuration
    
    var hiddenStorage: Any?
...
func doSomethingInternally(with configuration: Configuration) {
    ...
    if someCondition() {
        configuration.hiddenStorage = 10
    }
}
```

You can still use switch-case internally:
