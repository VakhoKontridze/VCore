...
When an `case` has associated values, it cannot be marked as potentially unavailable with the `@available` attribute, and attempting to do so will result in a compiler error. This is because the availability of a `case` with associated values cannot be determined until the associated values themselves have been evaluated at runtime.
