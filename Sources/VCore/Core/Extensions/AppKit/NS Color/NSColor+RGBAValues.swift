...
nonisolated extension NSColor {
    ...
    public var rgbaValues: (red: CGFloat, green: CGFloat, blue: CGFloat, alpha: CGFloat) {
        let color: NSColor = {
            if let calibratedColor: NSColor = usingColorSpace(.deviceRGB) {
                ...
                Logger.default.error("Failed to calibrate 'NSColor' '\(self.debugDescription)' with 'NSColorSpace.deviceRGB'") // swiftlint:disable:this redundant_self
                return self
            }
        }()
    ...
    public func isRGBAEqual(to otherColor: NSColor) -> Bool {
        VCore::isEqual(
            rgbaValues,
            to: otherColor.rgbaValues,
            by: \.red, \.green, \.blue, \.alpha
        )
    }
}
